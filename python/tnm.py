...
def tnm(Xp: np.ndarray, q: np.ndarray, K: np.ndarray) -> [np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """
    ...
    if N > 3:
        for i in range(M):
            ...
            retval, rvec, tvec = cv2.solvePnP(mXp, np.ascontiguousarray(mqi).reshape((mN, 1, 2)), K, None, flags=cv2.SOLVEPNP_ITERATIVE)
        ...
        for i in range(M):
            # P3P fails ...
            retval, rvecs, tvecs = cv2.solveP3P(Xp, np.ascontiguousarray(q[i]).reshape((N, 1, 2)), K, None, flags=cv2.SOLVEPNP_P3P)
